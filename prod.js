...
(function () {
    ...
    // ======================================================
        ...
        <div class="utst-bubble-settings" style="margin-top: 14px; padding-top: 14px; border-top: 1px solid rgba(255, 255, 255, 0.1); display:flex; flex-direction:column; gap:10px;">
            ...
            <div style="font-size: 11px; color: rgba(255,255,255,0.5);">Можно разбить нагрузку например, сообщения лида переводить через ИИ (качественнее), а поля ввода  через Google (быстро,но хуево).</div>

            <label for="panelTranslateEngine" style="color:#fff; font-size:12px;">Перевод сообщений лида (панель, Ctrl+A):</label>
            ...
            <div id="aiSettingsBlock" style="display: none; flex-direction: column; gap: 10px; margin-top: 4px;">
                ...
                <label style="display:flex; align-items:center; gap:8px; color:#fff; font-size:12px; cursor:pointer; margin-top:2px;">
                    ...
                    Если модель недоступна - автоматически пингануть некст ии
                </label>
                </div>
                ...
                <div style="font-size: 11px; color: rgba(255,255,255,0.45);">Ключ берётся на build.nvidia.com → любая модель → "Get API Key"</div>
                ...
                <div style="font-size: 11px; color: rgba(255,255,255,0.45);">Указать тут другую модель (брать на build.nvidia.com)</div>
                </div>
                ...
                <div style="font-size: 11px; color: rgba(255,255,255,0.5);">Используй {target_lang} для автоматической подстановки языка перевода</div>
            </div>
        </div>
        <div class="utst-bubble-settings" style="margin-top: 14px; padding-top: 14px; border-top: 1px solid rgba(255, 255, 255, 0.1); display:flex; flex-direction:column; gap:10px;">
            <label id="fieldShortcutCaptureLabel" for="fieldShortcutCaptureButton" style="color:#fff; font-size:14px; display:block; margin-bottom:4px;">Перевод текста в полях ввода (чаты)</label>
            <div style="font-size: 11px; color: rgba(255,255,255,0.5); margin-bottom: 10px;">Хоткей переводит текст прямо в поле ввода.Работает независимо от основного хоткея выделения, чтобы не конфликтовать с отправкой сообщения на сайте(Но можно назначить одинаковые хоткеи для удобства)</div>
            <div class="utst-shortcut-control">
        ...
        <div class="utst-bubble-settings" style="margin-top: 14px; padding-top: 14px; border-top: 1px solid rgba(255, 255, 255, 0.1); display:flex; flex-direction:column; gap:10px;">
            ...
            <div style="font-size: 11px; color: rgba(255,255,255,0.5);">Один раз укажите область на сайте (например, панель с сообщениями лида) и скрипт будет переводить в ней всё, включая новые сообщения, которые появляются позже, без повторных действий, Настройка привязана к текущему домену.</div>
        ...
        <div class="utst-bubble-settings" style="margin-top: 14px; padding-top: 14px; border-top: 1px solid rgba(255, 255, 255, 0.1); display:flex; flex-direction:column; gap:10px;">
            ...
            <div style="font-size: 11px; color: rgba(255,255,255,0.5); margin-bottom: 10px;">При переводе выделенного сообщения дополнительно предлагается готовый вариант ответа</div>
            ...
            <input type="text" id="replySuggestionPoolUrl" class="utst-blacklist-input" placeholder="Ссылка на базу в формате raw" style="width:100%; box-sizing:border-box;">
            <div style="font-size: 11px; color: rgba(255,255,255,0.45); margin: 4px 0 8px;">Формат JSON, на одну тему можно указать сколько угодно вариантов ответа <span style="font-family:monospace;">[{"keywords":["как дела","how are you"],"replies":["Вариант 1...","Вариант 2...","Вариант 3..."]}]</span>. Тема выбирается по наибольшему числу совпавших ключевых слов в исходном сообщении. Старый формат с одиночным полем <span style="font-family:monospace;">"reply"</span> тоже понимается.</div>
            <div style="max-width:260px; margin:0 auto 6px;">
                <button id="replySuggestionPoolRefresh" type="button" class="utst-shortcut-capture" style="width:100%;">Обновить базу</button>
            </div>
    ...
    `;
        ...
        function updateFieldShortcutSettingsUi(message = '') {
            ...
            if (fieldShortcutCaptureHelp) {
                fieldShortcutCaptureHelp.textContent = message || 'Нажми, затем введи сочетание с Ctrl, Alt, Shift';
            }
        }
        ...
        function updatePageShortcutSettingsUi(message = '') {
            ...
            if (help) help.textContent = message || 'Нажмите, затем введите сочетание с Ctrl, Alt, Shift';
        }
